...
description: prottype の PR Preview 用 GitHub Secrets（CLOUDFLARE_API_TOKEN, CLOUDFLARE_ACCOUNT_ID）登録手順。 wrangler whoami で Account ID を取得し、gh secret set で登録。API トークン作成は Cloudflare ダッシュボードで行う。 PR Preview / GitHub Actions preview ジョブの初回セットアップ時に使う。
---
